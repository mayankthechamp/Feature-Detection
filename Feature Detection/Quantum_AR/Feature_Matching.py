...
if __name__ == '__main__':
    ...
    # Trying to open the camera from different indices
    camera_found = False
    for cam_index in range(10):  # Adjust range based on the number of connected cameras
        cap = cv2.VideoCapture(cam_index)
        if cap.isOpened():
            print(f"Camera opened with index {cam_index}")
            camera_found = True
            break
    if not camera_found:
